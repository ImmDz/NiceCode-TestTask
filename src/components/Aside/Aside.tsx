...
export const Aside = () => {
    ...
    const choosePatient = (id: string) => {
        ...
        if (choosenPatient) {
            if (isActivePickButton) {
                setPickedPatientIds((prev) => prev.includes(id) ? prev.filter((item) => item !== id) : [...prev, id]);
    ...
    const deletePatient = () => {
        const filteredPatients = patientState.patients.filter((patient) => !pickedPatientIds.includes(patient.id));
    ...
    const renderCounter = () => {
        return isActivePickButton ?
            <>
                ...
                <span className={classNames(style.asideCounter, { [style.active]: isActivePickButton })}>{pickedPatientIds.length}</span>
            </> :
    ...
    return (
        <aside>
            ...
            {filterValue ? <ul className={style.asideList}>{patientState.filteredPatients.map((patient) =>
                <li key={patient.id}>
                    <PatientCard id={patient.id} pickedPatients={pickedPatientIds} isActivePickButton={isActivePickButton} name={patient.name} surname={patient.surname} img={patient.img} onClick={() => choosePatient(patient.id)} />
                </li>)}
            </ul> : <ul className={style.asideList}>{patientState.patients.map((patient) =>
                <li key={patient.id}>
                    <PatientCard id={patient.id} pickedPatients={pickedPatientIds} isActivePickButton={isActivePickButton} name={patient.name} surname={patient.surname} img={patient.img} onClick={() => choosePatient(patient.id)} />
                </li>)}
            </ul>}
