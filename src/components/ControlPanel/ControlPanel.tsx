...
export const ControlPanel = ({ filterValue, setFilterValue }: IControlPanel) => {
    ...
    const filterPatients = (name: string) => {
        setFilterValue(name);
        const filteredPatients = patientState.patients.filter((patient) => patient.name.toLowerCase().includes(name.toLowerCase()));
    ...
    return (
        <div className={style.panelContainer} >
            ...
            <div className={style.buttonGroup}>
                <button className={style.sort} onClick={sortPatients}><Sort /></button>
                <button type="button" onClick={() => dispatch(toggle(true))} className={style.add}><Plus /></button>
            </div>
        </div>
    )
};